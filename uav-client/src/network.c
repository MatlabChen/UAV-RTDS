...
void forward_network(network net, network_state state)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        if(l.delta && state.train){
            // double scal_cpu_time = get_time_point();
            scal_cpu(l.outputs * l.batch, 0, l.delta, 1);
            // printf("scal_cpu函数 in %lf milli-seconds.\n", ((double)get_time_point() - scal_cpu_time) / 1000);
        }
        // double time = get_time_point();
        l.forward(l, state);
        // printf("forward函数 in %lf milli-seconds.\n", ((double)get_time_point() - time) / 1000);
...
{
    double forward_network_start = get_time_point();
#ifdef GPU
    if(gpu_index >= 0)  {
        float *out = network_predict_gpu(net, input);
        printf("forward_network time = %lf\n", ((double)get_time_point() - forward_network_start) / 1000);
        return out;
    }
#endif
    ...
    double get_network_output_start = get_time_point();
    float *out = get_network_output(net);
    printf("get_network_output time = %lf\n", ((double)get_time_point() - get_network_output_start) / 1000);
