...
{
    double test_detector_start = get_time_point();

    double read_data_cfg_start = get_time_point();
    list *options = read_data_cfg(datacfg);
    printf("read_data_cfg time = %lf\n", ((double)get_time_point() - read_data_cfg_start) / 1000);

    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    char **names = get_labels_custom(name_list, &names_size); //get_labels(name_list);

    double load_alphabet_start = get_time_point();
    image **alphabet = load_alphabet();
    printf("load_alphabet time = %lf\n", ((double)get_time_point() - load_alphabet_start) / 1000);

    double parse_network_cfg_custom_start= (double)get_time_point();
    network net = parse_network_cfg_custom(cfgfile, 1, 1); // set batch=1
    printf("parse_network_cfg_custom time = %lf\n", ((double)get_time_point() - parse_network_cfg_custom_start) / 1000);

    if (weightfile) {
        double load_weights_start = get_time_point();
        load_weights(&net, weightfile);
        printf("load_weights time = %lf\n", ((double)get_time_point() - load_weights_start) / 1000);
    }
    net.benchmark_layers = benchmark_layers;

    double fuse_conv_batchnorm_start = get_time_point();
    fuse_conv_batchnorm(net);
    printf("fuse_conv_batchnorm time = %lf\n", ((double)get_time_point() - fuse_conv_batchnorm_start) / 1000);

    double calculate_binary_weights_start = (double)get_time_point();
    calculate_binary_weights(net);
    printf("calculate_binary_weights time = %lf\n", ((double)get_time_point() - calculate_binary_weights_start) / 1000);
    ...
    while (1) {
        ...
        int nboxes = 0;
        double get_network_boxes_start = get_time_point();
        detection *dets = get_network_boxes(&net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letter_box);
        printf("get_network_boxes time = %lf\n", ((double)get_time_point() - get_network_boxes_start) / 1000);
        ...
        double draw_detections_start = get_time_point();
        draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output);
        printf("draw_detections time = %lf\n", ((double)get_time_point() - draw_detections_start) / 1000);
    ...
    printf("总耗时 time = %lf ms\n", ((double)get_time_point() - test_detector_start) / 1000);
